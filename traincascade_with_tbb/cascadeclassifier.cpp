                                ...
                                bool baseFormatSave )
{
    ...
    if ( !imgReader.create( _posFilename, _negFilename, _cascadeParams.winSize, omp_get_max_threads()) )
    {
...
#pragma omp critical
				...
				{
					isGetImg = imgReader.getNeg( img , thread_id );
				}
#pragma omp critical
				{
					if (isGetImg)
...
#pragma omp critical
						{
							result = true;
							getcount++;
							printf("%s current samples: %d, thread id:%d\r", isPositive ? "POS":"NEG", getcount, omp_get_thread_num());
						}
